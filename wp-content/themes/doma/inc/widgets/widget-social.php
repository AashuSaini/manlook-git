...
if (!class_exists('ZooSocialWidget')) {
    class ZooSocialWidget extends WP_Widget
    {
        ...
        {
            $widget_ops = array('classname' => 'ZooSocialWidget', 'description' => esc_html__('Displays your social profile.', 'doma'));

            parent::__construct(false, esc_html__('Zoo: Social profile', 'doma'), $widget_ops);
        }
        ...
        function form($instance)
        {
            ...
            <p>
                <label
                    for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php echo esc_html__('Title', 'doma'); ?></label>
            ...
            <p>
                <label
                    for="<?php echo esc_attr($this->get_field_id('mode')); ?>"><?php echo esc_html__('Display', 'doma'); ?></label>
                <select id="<?php echo esc_attr($this->get_field_id('mode')); ?>"
                        name="<?php echo esc_attr($this->get_field_name('mode')); ?>" style="width:100%;">
                    <option
                        value='text' <?php if ('text' == $instance['mode'] || $instance['mode'] == '') echo 'selected="selected"'; ?>><?php echo esc_html__('Only Text', 'doma'); ?></option>
                    <option
                        value='icon' <?php if ('icon' == $instance['mode']) echo 'selected="selected"'; ?>><?php echo esc_html__('Only Icon', 'doma'); ?></option>
                    <option
                        value='both' <?php if ('both' == $instance['mode']) echo 'selected="selected"'; ?>><?php echo esc_html__('Both Text and Icon', 'doma'); ?></option>
                </select>
            </p> <?php
            foreach ($this->socials as $key => $social) {
                ?>
