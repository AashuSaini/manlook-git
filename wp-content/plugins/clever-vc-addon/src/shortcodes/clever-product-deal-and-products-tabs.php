<?php
/**
 * Product Deal and Products Tabs Shortcode
 */
if (!function_exists('cvca_shortcode_products_deal_and_tabs')) {
    function cvca_shortcode_products_deal_and_tabs($atts, $content)
    {

        if (class_exists('WooCommerce')) {
            $atts = shortcode_atts(apply_filters('CleverProductDealandTabs_shortcode_atts',array(
                'title' => '',
                'accent_color' => '',
                ...
                'css' => '',
            )), $atts, 'CleverProductDealandTabs');
            $html = cvca_get_shortcode_view( 'product-deal-and-products-tabs', $atts, $content );
...
add_shortcode('CleverProductDealandTabs', 'cvca_shortcode_products_deal_and_tabs');

add_action('vc_before_init', 'cvca_products_deal_and_tabs_integrate_vc');

if (!function_exists('cvca_products_deal_and_tabs_integrate_vc')) {
    function cvca_products_deal_and_tabs_integrate_vc()
    {
        if (class_exists('WooCommerce')) {
            $asset_type =  array(
            ...
            vc_map(
                array(
                    'name' => esc_html__('Product Deal and Products Tabs', 'cvca'),
                    'base' => 'CleverProductDealandTabs',
                    'icon' => 'icon-wpb-woocommerce',
                    'category' => esc_html__('CleverSoft Elements', 'cvca'),
                    'description' => esc_html__('Show product deal and tabs', 'cvca'),
                    'params' => array(
                        array(
                            'type' => 'textfield',
                        ...
                        array(
                            ...
                            'heading' => esc_html__('Asset type for tabs', 'cvca'),
                            'value' => $asset_type,
                            'std' => '',
                        ...
                        array(
                            'type' => 'dropdown',
                            'heading' => esc_html__('Default Asset type', 'cvca'),
                            'value' => $asset_type,
                            'std' => '',
                            'param_name' => 'default_asset_type',
                            'description' => esc_html__('Select default asset type of products for show filter', 'cvca'),
                        ),
                        array(
                            'type' => 'autocomplete',
                        ...
                        array(
                            "type" => "cvca_multiselect2",
                            "heading" => esc_html__("Choose categories", 'cvca'),
                        ...
                        array(
                            'type' => 'textfield',
                            'heading' => esc_html__('Title of Deal', 'cvca'),
                            "admin_label" => true,
                        ...
                        array(
                            'type' => 'textfield',
                            'heading' => esc_html__('Enter Product Deal ID', 'cvca'),
                            'std' => '',
                            'param_name' => 'deal_id',
                            'description' => esc_html__('Enter product deal IDs for show ', 'cvca'),
                        ),
                        array(
                            'type' => 'checkbox',
...
add_filter( 'vc_autocomplete_CleverProductDealandTabs_product_ids_callback', array(
    $wc_class,
    'productIdAutocompleteSuggester',
), 10, 1 ); // Get suggestion(find). Must return an array
add_filter( 'vc_autocomplete_CleverProductDealandTabs_product_ids_render', array(
    $wc_class,
    'productIdAutocompleteRender',
), 10, 1 ); // Render exact product. Must return an array (label,value)
//For param: ID default value filter
add_filter( 'vc_form_fields_render_field_CleverProductDealandTabs_product_ids_param_value', array(
    $wc_class,
