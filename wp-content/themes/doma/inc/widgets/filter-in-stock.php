 ...
 * @see   WP_Widget
 */
add_action( 'widgets_init', 'zoo_load_clever_filter_in_stock' );

function zoo_load_clever_filter_in_stock() {
    register_widget( 'clever_filter_in_stock' );
}
class clever_filter_in_stock extends WP_Widget {
	protected $defaults;
	...
	function __construct() {
		...
		parent::__construct(
			'clever_filter_in_stock',
			esc_html__( 'Zoo: Filter by In Stock', 'doma' ),
			array(
				'classname'   => 'woocommerce widget_text clever-filter-in-stock',
				'description' => esc_html__( 'Filter by In Stock', 'doma' ),
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( isset($_GET['availability']) && $_GET['availability'] == 'in_stock' ) {
			$output .= '<li class="chosen"><a href="' . esc_url( $link ) . '"><span></span>' . esc_html__( 'In stock only', 'doma' ) . '</a></li>';
		} else {
			$url = add_query_arg( array( 'availability' => 'in_stock' ), $link );
			$output .= '<li><a href="' . esc_url( $url ) . '"><span></span>' . esc_html__( 'In stock only', 'doma' ) . '</a></li>';
