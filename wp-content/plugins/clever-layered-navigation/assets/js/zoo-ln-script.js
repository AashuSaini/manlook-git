(function ($) {
    ...
    jQuery(document).ready(function ($) {
        ...
        {
            // let shopUrl = form.data('ln-config').shop_page_info.url,
            //     formParams = get_url_params(form.serializeArray());
            //
            // if (-1 !== shopUrl.indexOf('?')) {
            //     formParams = formParams.replace('?', '&');
            // }
            //
            // const newUrl = shopUrl + formParams;
            ...
            const newUrl = url + formParams;

            window.history.pushState({path: newUrl}, '', newUrl);

            if (form.hasClass('apply_ajax')) {
                zoo_ln_ajax(form);
            } else {
            ...
            $.post(ajax_url, data, function (result) {
                ...
                if (typeof html_pagination_content != 'undefined') {
                    if (html_pagination_content.length == 0) {
                        ...
                        if ($(jquery_selector_paging)[0]) {
                            $(jquery_selector_paging).show().replaceWith(html_pagination_content);
                            $(jquery_selector_paging).addClass('zoo_ln_ajax_pagination');
                            $(jquery_selector_paging).attr('data-ln-preset', ajax_params.filter_preset);
                        }
                        else
                            $(jquery_selector_products).after(html_pagination_content);
                    }
                }
