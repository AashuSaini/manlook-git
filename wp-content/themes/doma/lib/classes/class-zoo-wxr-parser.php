...
class ZooWXRParser
{
  ...
  function parse($file)
  {
    ...
    if (empty($file)) {
      return new WP_Error('WXR_parse_error', esc_html__('Invalid WXR file.', 'zoo-logistic'));
    }

    // Cleanup invalid encoding chars.
    $xml_content = preg_replace('/[^\x9\xa\x20-\xD7FF\xE000-\xFFFD]/', '', file_get_contents($file));

    if (extension_loaded('simplexml')) {
      return $this->simplexmlParse($xml_content);
    } elseif (extension_loaded('xmlreader')) {
      return $this->xmlreaderParse($xml_content);
    } else {
      return new WP_Error('WXR_parse_error', esc_html__('No XML parser found.', 'zoo-logistic'));
    }
  }
   ...
   */
  protected function simplexmlParse($xml_content)
  {
    ...
    if (!$xml) {
      return new WP_Error('WXR_parse_error', esc_html__('Invalid WXR file.', 'zoo-logistic'), libxml_get_errors());
    }
   ...
   */
  protected function xmlreaderParse($xml_content)
  {
    $data = array();
    $reader = new XMLReader();
    $status = $reader->xml($xml_content);

    if (!$status) {
      return new WP_Error('WXR_parse_error', esc_html__('Could not read the XML content.', 'zoo-logistic'));
    }
