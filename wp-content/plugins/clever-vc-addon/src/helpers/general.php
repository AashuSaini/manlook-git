 ...
 *
 * @see  CVCA\Helpers\ShortcodeFactory::create()
 *
 * @api
 */
...
/* ================= Add Shortcode============ */
/* Shortcode menu categories*/
if(!function_exists('shortcode_menu_categories')){
    function shortcode_menu_categories($args) {
            ob_start();
            ...
            ?>
            <div class="categories-menu-inner <?php if(get_post_meta(get_the_ID(),'zoo_categories_menu', true) == '1'){ echo 'open';} ?>">
            <label class="categories-label" style="<?php echo esc_attr( $style); ?>"><i class="zoo-css-icon-menu"></i><?php echo esc_attr( $title ); ?></label>
...
add_shortcode( 'menu_categories', 'shortcode_menu_categories' );

/* Shortcode footer brand */
if(!function_exists('shortcode_footer_brand')){
    function shortcode_footer_brand($args) {
            ob_start();
            $pays = get_theme_mod( 'zoo_footer_bottom_right' ); 
            if($pays):
                echo '<ul class="pay-list">';
                foreach( $pays as $pay ) :
                    $image_url = wp_get_attachment_image( $pay['brand_image']['id'], array( 57, 58 ), false, array( "class" => "svg-icon" ) );
                    $url = $pay['brand_link'];
                    ?>
                    <li class="payment-icon">
                        <a href="<?php echo esc_url($url); ?>" title="<?php echo esc_attr($pay['brand_title']); ?>">
...
add_shortcode( 'footer_brand', 'shortcode_footer_brand' );