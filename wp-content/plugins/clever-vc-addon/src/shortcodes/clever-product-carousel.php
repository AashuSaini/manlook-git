 ...
 */
if (!function_exists('cvca_shortcode_products_carousel')) {
    function cvca_shortcode_products_carousel($atts, $content)
    {

        if (class_exists('WooCommerce')) {
            $atts = shortcode_atts(apply_filters('CleverProductCarousel_shortcode_atts',array(
                'title' => '',
                'accent_color' => '',
                ...
                'css' => '',
            )), $atts, 'CleverProductCarousel');
            $html = cvca_get_shortcode_view( 'product-carousel', $atts, $content );
...
add_shortcode('CleverProductCarousel', 'cvca_shortcode_products_carousel');

add_action('vc_before_init', 'cvca_product_carousel_integrate_vc');

if (!function_exists('cvca_product_carousel_integrate_vc')) {
    function cvca_product_carousel_integrate_vc()
    {
        if (class_exists('WooCommerce')) {
            $asset_type =  array(
            ...
            vc_map(
                array(
                    'name' => esc_html__('Clever Products Carousel', 'cvca'),
                    'base' => 'CleverProductCarousel',
...
add_filter( 'vc_autocomplete_CleverProductCarousel_product_ids_callback', array(
    $wc_class,
    'productIdAutocompleteSuggester',
), 10, 1 ); // Get suggestion(find). Must return an array
add_filter( 'vc_autocomplete_CleverProductCarousel_product_ids_render', array(
    $wc_class,
    'productIdAutocompleteRender',
), 10, 1 ); // Render exact product. Must return an array (label,value)
//For param: ID default value filter
add_filter( 'vc_form_fields_render_field_CleverProductCarousel_product_ids_param_value', array(
    $wc_class,
