...
<?php do_action('yith_wcwl_before_wishlist_form', $wishlist_meta); ?>
    ...
    <form id="yith-wcwl-form" action="<?php echo ent2ncr($form_action) ?>" method="post" class="woocommerce">
        ...
        if (!empty($page_title)) :
            ?>
            <div class="wishlist-title <?php echo ent2ncr($is_custom_list) ? 'wishlist-title-with-form' : '' ?>">
                ...
                <?php if ($is_custom_list): ?>
                    <a class="btn button show-title-form">
                        ...
                        <?php _e('Edit title', 'doma') ?>
                    </a>
                <?php endif; ?>
            </div>
            <?php if ($is_custom_list): ?>
            <div class="hidden-title-form">
                <input type="text" value="<?php echo ent2ncr($page_title) ?>" name="wishlist_name"/>
                <button>
        ...
        <table class="shop_table cart wishlist_table" data-pagination="<?php echo esc_attr($pagination) ?>"
               ...
               data-id="<?php echo ent2ncr($wishlist_id) ?>" data-token="<?php echo ent2ncr($wishlist_token) ?>">
            ...
            <tr>
                ...
                <th class="product-name" colspan="2">
                    <span class="nobr"><?php echo apply_filters('yith_wcwl_wishlist_view_name_heading', __('Product Name', 'doma')) ?></span>
                </th>
                ...
                <?php if ($show_price) : ?>
                    ...
                    <span class="nobr">
                        <?php echo apply_filters('yith_wcwl_wishlist_view_price_heading', __('Unit Price', 'doma')) ?>
                    </span>
                    </th>
                ...
                <?php if ($show_stock_status) : ?>
                    ...
                    <span class="nobr">
                        <?php echo apply_filters('yith_wcwl_wishlist_view_stock_heading', __('Stock Status', 'doma')) ?>
                    </span>
                    </th>
            ...
            if (count($wishlist_items) > 0) :
                ...
                foreach ($wishlist_items as $item) :
                    ...
                    if ($product && $product->exists()) :
                        ?>
                        <tr id="yith-wcwl-row-<?php echo ent2ncr($item['prod_id']) ?>"
                            data-row-id="<?php echo ent2ncr($item['prod_id']) ?>">
                            <?php if ($show_cb) : ?>
                                <td class="product-checkbox">
                                    <input type="checkbox" value="<?php echo esc_attr($item['prod_id']) ?>"
                                           name="add_to_cart[]" <?php echo ent2ncr((!$product->is_type('simple')) ? 'disabled="disabled"' : '' )?>/>
                                </td>
                            <?php endif ?>
                            ...
                            <?php if ($show_price) : ?>
                                <td class="product-price">
                                    <?php echo ent2ncr($product->get_price() ? $product->get_price_html() : apply_filters('yith_free_text', __('Free!', 'doma'))); ?>
                                </td>
                            <?php endif ?>

                            <?php if ($show_stock_status) : ?>
                                <td class="product-stock-status">
                                    <?php echo ent2ncr($stock_status == 'out-of-stock' ? '<span class="wishlist-out-of-stock">' . __('Out of Stock', 'doma') . '</span>' : '<span class="wishlist-in-stock">' . __('In Stock', 'doma') . '</span>'); ?>
                                </td>
                            <?php endif ?>
                            ...
                            <?php if ($show_last_column): ?>
                                <td class="product-add-to-cart">
                                    ...
                                    if ($show_dateadded && isset($item['dateadded'])):
                                        echo '<span class="dateadded">' . sprintf(__('Added on : %s', 'doma'), date_i18n(get_option('date_format'), strtotime($item['dateadded']))) . '</span>';
                                    endif;
                                    ?>
                                    ...
                                    <?php if ($available_multi_wishlist && is_user_logged_in() && count($users_wishlists) > 1 && $move_to_another_wishlist && $is_user_owner): ?>
                                        <select class="change-wishlist selectBox">
                                            <option value=""><?php _e('Move', 'doma') ?></option>
                                            <?php
                                            foreach ($users_wishlists as $wl):
                                                ...
                                                <option value="<?php echo esc_attr($wl['wishlist_token']) ?>">
                                                    ...
                                                    if ($wl['wishlist_privacy'] == 1) {
                                                        $wl_privacy = __('Shared', 'doma');
                                                    } elseif ($wl['wishlist_privacy'] == 2) {
                                                        $wl_privacy = __('Private', 'doma');
                                                    } else {
                                                        $wl_privacy = __('Public', 'doma');
                                                    }
                                    ...
                                    <?php if ($is_user_owner && $repeat_remove_button): ?>
                                        <a href="<?php echo esc_url(add_query_arg('remove_from_wishlist', $item['prod_id'])) ?>"
                                           ...
                                           title="<?php _e('Remove this product', 'doma') ?>"><?php _e('Remove', 'doma') ?></a>
                                    <?php endif; ?>
                                </td>
                            <?php endif; ?>
                            <?php if ($is_user_owner): ?>
                                <td class="product-remove">
                                    <div>
                                        <a href="<?php echo esc_url(add_query_arg('remove_from_wishlist', $item['prod_id'])) ?>"
                                           ...
                                           title="<?php _e('Remove this product', 'doma') ?>">&times;</a>
                                    </div>
                                </td>
                            <?php endif; ?>
                        </tr>
                ...
                <tr>
                    <td colspan="<?php echo esc_attr($column_count) ?>"
                        class="wishlist-empty"><?php echo apply_filters('yith_wcwl_no_product_to_remove_message', __('No products were added to the wishlist', 'doma')) ?></td>
                </tr>
                <?php
            endif;
            ...
            if (!empty($page_links)) : ?>
                <tr class="pagination-row">
                    <td colspan="<?php echo esc_attr($column_count) ?>"><?php echo ent2ncr($page_links) ?></td>
                </tr>
            <?php endif ?>
            </tbody>
        </table>
            ...
            <?php if ($show_cb) : ?>
                <div class="custom-add-to-cart-button-cotaniner">
                    <a href="<?php echo esc_url(add_query_arg(array('wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_token))) ?>"
                       class="button alt"
                       id="custom_add_to_cart"><?php echo apply_filters('yith_wcwl_custom_add_to_cart_text', __('Add the selected products to the cart', 'doma')) ?></a>
                </div>
            <?php endif; ?>
                ...
                <div class="ask-an-estimate-button-container">
                    <a href="<?php echo ($additional_info || !is_user_logged_in()) ? '#ask_an_estimate_popup' : $ask_estimate_url ?>"
                       class="btn button ask-an-estimate-button" <?php echo ($additional_info) ? 'data-rel="prettyPhoto[ask_an_estimate]"' : '' ?> >
                        ...
                        <?php echo apply_filters('yith_wcwl_ask_an_estimate_text', __('Ask for an estimate', 'doma')) ?>
                    </a>
                </div>
            <?php endif; ?>
        ...
        <?php if (!$is_default): ?>
            <input type="hidden" value="<?php echo ent2ncr($wishlist_token) ?>" name="wishlist_id" id="wishlist_id">
        <?php endif; ?>
...
<?php if ($show_ask_estimate_button && (!is_user_logged_in() || $additional_info)): ?>
    <div id="ask_an_estimate_popup">
        <form action="<?php echo ent2ncr($ask_estimate_url) ?>" method="post" class="wishlist-ask-an-estimate-popup">
            <?php if (!is_user_logged_in()): ?>
                <label for="reply_email"><?php echo apply_filters('yith_wcwl_ask_estimate_reply_mail_label', __('Your email', 'doma')) ?></label>
