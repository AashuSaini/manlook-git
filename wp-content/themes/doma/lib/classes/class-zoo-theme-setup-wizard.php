...
if (!class_exists('ZooThemeSetupWizard', false)) :
	...
	class ZooThemeSetupWizard
	{
		...
		function _setup_wizard()
		{
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text'  => esc_html__( ' Verifying', 'doma' ),
			) );
		...
		function _ajax_child_theme()
		{
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['cThemeName'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'Invalid child theme data. Please try again!', 'doma' ) ) );
			}
	 ...
	 */\n";
			...
			if ( !file_exists($child_theme_path) ) {
				...
				wp_send_json( array( 'done' => 1, 'message' => sprintf( esc_html__('%s has already been created. Child theme files are stored in %s.', 'doma' ), $child_theme_title, '<code>' . $child_theme_path . '</code>' ) ) );
			}
			...
			wp_send_json( array( 'done' => 1, 'message' => sprintf( esc_html__('%s has been created and activated successfully! Child theme files are stored in %s.', 'doma' ), $child_theme_title, '<code><strong>' . $child_theme_path . '</strong></code>' ) ) );
		}
		...
		function _ajax_plugins()
		{
			...
			foreach ( $plugins['activate'] as $slug => $plugin ) {
				if ( $_POST['slug'] === $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Activating', 'doma' ),
					);
					break;
				}
			}
			...
			foreach ( $plugins['update'] as $slug => $plugin ) {
				if ( $_POST['slug'] === $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Updating', 'doma' ),
					);
					break;
				}
			}
			...
			foreach ( $plugins['install'] as $slug => $plugin ) {
				if ( $_POST['slug'] === $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Installing', 'doma' ),
					);
					break;
				}
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success', 'doma' ) ) );
			}
		...
		function _ajax_content()
		{
			...
			if ( !check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[$_POST['content']] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'Invalid content!', 'doma' ) ) );
			}
		...
		protected function setup_wizard_header()
		{
			...
			<head>
				...
				<?php printf( esc_html__( '%s%sTheme &rsaquo; Setup Wizard%s', 'doma' ), '<ti', 'tle>', '</title>' ) ?>
		...
		{
			?><a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'doma' ); ?></a>
			</body><?php
		...
		protected function introduction()
		{
			if ( $this->is_possible_upgrade() ) :
				?><h1><?php esc_html_e( 'Welcome to theme setup wizard!', 'doma' ); ?></h1>
				<p><?php esc_html_e( 'It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.', 'doma' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large"><?php esc_html_e( 'Let&#8217;s go!', 'doma' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url() ); ?>"
					   class="button button-large"><?php esc_html_e( 'Not right now', 'doma' ); ?></a>
				</p>
				<?php
			elseif ( get_option( $this->theme->template . '_theme_setup_completed' ) && true === $this->tgmpa->is_tgmpa_complete() ) :
				?>
				<h1><?php esc_html_e( 'Welcome to theme setup wizard!', 'doma' ); ?></h1>
				<p class="lead success"><strong><?php esc_html_e( 'It looks like you have already run the setup wizard. If you want to run the wizard again, click on "Run setup wizard again".', 'doma' ); ?></strong></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large"><?php esc_html_e( 'Run setup wizard again', 'doma' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large"><?php esc_html_e( 'Skip this step', 'doma' ); ?></a>
				...
				?>
				<h1><?php esc_html_e( 'Welcome to theme setup wizard!', 'doma' ); ?></h1>
				<p><?php printf(esc_html__( 'Thank you for choosing %s theme! This setup wizard is to help you setup your new website quickly. Step by step, you will install necessary plugins, import demo content, customize site appearance...', 'doma' ), $this->theme->name); ?></p>
				<p><?php esc_html_e( 'If you don&#8217;t want to go through the wizard, you can skip any step or return to the WordPress admin dashboard. Come back anytime if you change your mind!', 'doma' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large"><?php esc_html_e( 'Let&#8217;s go!', 'doma' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url() ); ?>" class="button button-large"><?php esc_html_e( 'Not right now', 'doma' ); ?></a>
				</p>
				<?php
			endif;
		}
		...
		{
			?><p><?php esc_html_e('Before installing the theme, we need to check if your system environment. The theme will not work properly if your system doen not meet minimal requirements. Please check it carefully!', 'doma') ?></p>
			<table class="theme-setup-system-status" style="width:100%;padding-bottom:20px;">
				<thead>
					<th><?php esc_html_e('Environment', 'doma') ?></th>
					<th><?php esc_html_e('Info', 'doma') ?></th>
					<th><?php esc_html_e('Value', 'doma') ?></th>
					<th><?php esc_html_e('Status', 'doma') ?></th>
				</thead>
				<tbody>
	        <tr>
	            <?php
	            ...
	            ?>
	            <td class="field-label"><?php esc_html_e('XML Parser', 'doma'); ?></td>
	            <td>
	                <span class="field-tooltip" href="#">
	                    <span class="dashicons dashicons-info"></span>
	                    <span class="tip-content">
	                        <?php printf(esc_html__('%s requires SimpleXML or XMLReader extension of PHP to parse WXR file. Make sure to install one of them before importing sample data!', 'doma'), $this->theme->name); ?>
	                    </span>
	                </span>
	            </td>
	            ...
	            <td class="field-status">
					...
					<tr>
						<td><?php esc_html_e('PHP Version', 'doma') ?></td>
						<td>
	            <span class="field-tooltip">
	                <span class="dashicons dashicons-info"></span>
	                <span class="tip-content">
	                    <?php printf( esc_html__('%s requires PHP version 5.4 at least.', 'doma'), $this->theme->name ); ?>
	                    <?php esc_html_e('You should use latest version for better perfomance and security.', 'doma'); ?>
	                </span>
	            </span>
						</td>
					...
					<tr>
						<td><?php esc_html_e('PHP Memory Limit', 'doma') ?></td>
						<td>
	                        <span class="field-tooltip">
	                            <span class="dashicons dashicons-info"></span>
	                            <span class="tip-content">
	                                <?php esc_html_e('Importing demo data could use large amount of memory.', 'doma'); ?>
									<?php printf( esc_html__('At peak, %s may require at least 256M of memory.', 'doma'), $this->theme->name ); ?>
	                            </span>
	                        </span>
						</td>
					...
					<tr>
						<td><?php esc_html_e('WordPress Version', 'doma') ?></td>
						<td>
	                        <span class="field-tooltip">
	                            <span class="dashicons dashicons-info"></span>
	                            <span class="tip-content">
	                                <?php printf( esc_html__('%s requires WordPress version 4.4 at least.', 'doma'), $this->theme->name ); ?>
	                                <?php esc_html_e('You should use latest version for better perfomance and security.', 'doma'); ?>
	                            </span>
	                        </span>
						</td>
	            ...
	            <td class="field-label"><?php
	                esc_html_e('Max Execution Time', 'doma');
	            ?></td>
	            <td>
	                <span class="field-tooltip" href="#">
	                    <span class="dashicons dashicons-info"></span>
	                    <span class="tip-content">
	                        <?php printf(esc_html__('%s requires at least 30s of PHP execution time.', 'doma'), $this->theme->name); ?>
	                    </span>
	                </span>
	            </td>
	        ...
	        <tr>
	            ...
	            ?>
	            <td class="field-label"><?php
	                esc_html_e('Max Upload File Size', 'doma');
	            ?></td>
	            <td>
	                <span class="field-tooltip" href="#">
	                    <span class="dashicons dashicons-info"></span>
	                    <span class="tip-content">
	                        <?php printf(esc_html__('%s requires at least 2M of total upload file size.', 'doma'), $this->theme->name); ?>
	                    </span>
	                </span>
	            </td>
	            ...
	            <td class="field-status">
	               ...
	               <tr>
	                    ...
	                    ?>
	                    <td class="field-label"><?php
	                        esc_html_e('Server Max Post Size', 'doma');
	                    ?></td>
	                    <td>
	                        <span class="field-tooltip" href="#">
	                            <span class="dashicons dashicons-info"></span>
	                            <span class="tip-content"><?php
	                                printf( esc_html__('%s requires at least 8M of total file size.', 'doma'), $this->theme->name );
	                            ?></span>
	                        </span>
	                    </td>
			...
			<?php if (!$this->system_status) : ?>
				<p style="color:red"><strong><?php esc_html_e('Whoops, your system does not meet the requirements. You should contact your hosting or VPS provider to fix it before installing this theme.', 'doma') ?></strong></p>
			<?php endif; ?>
			<p class="envato-setup-actions step">
				<?php
					if ($this->system_status) :
						?><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large"><?php esc_html_e( 'Continue', 'doma' ); ?></a><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large"><?php esc_html_e( 'Skip this step', 'doma' ); ?></a><?php
					else :
						?><a href="<?php echo esc_url( admin_url() ); ?>" class="button-primary button button-large"><?php esc_html_e( 'Abort', 'doma' ); ?></a><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large"><?php esc_html_e( 'Proceed anyway', 'doma' ); ?></a><?php
					endif;
				?>
			</p>
			<?php
		...
		protected function child_theme()
		{
			...
			?><h1><?php esc_html_e('Setup a Child Theme', 'doma') ?></h1>
			<p id="envato-setup-child-theme-text" class="<?php echo esc_attr($class_attr) ?>"><?php
				if ($is_child_theme) {
					printf( esc_html__('%sGreat! A child theme named as %s has already been created and activated successfully%s.', 'doma'), '<strong>', $theme, '</strong>' );
				} else {
					printf( esc_html__('If you are going to make changes to theme source code, it&#8217;s better to use a %schild theme%s rather than modifying parent theme. This allows parent theme to receive updates without overwriting your modifications. To generate a child theme, please enter child theme name then click on the "Create child theme" button.', 'doma'), '<a href="https://codex.wordpress.org/Child_Themes" target="_blank">', '</a>' );
				}
			?></p>
			<form action="" method="post">
			 	<div class="child-theme-input" style="margin-bottom:20px;">
			 		<label for="child_theme_name" style="margin-bottom: 5px; display: block;"><strong><?php esc_html_e('Child Theme Name', 'doma') ?></strong></label>
				...
				<p class="envato-setup-actions step"><?php
					if (!$is_child_theme) :
				    	?><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_child"><?php esc_html_e( 'Create child theme', 'doma' ); ?></a>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large"><?php esc_html_e( 'Skip this step', 'doma' ); ?></a><?php
						wp_nonce_field('envato-setup');
					else :
						?><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large"><?php esc_html_e( 'Continue', 'doma' ); ?></a><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large"><?php esc_html_e( 'Skip this step', 'doma' ); ?></a>
		...
		protected function default_plugins()
		{
			...
			?><h1><?php esc_html_e( 'Setup Plugins', 'doma' ); ?></h1>
			<form action="" method="post"><?php
				$plugins = $this->get_tgmpa_plugins();
				$count = count( $plugins['all'] );
				if ( $count ) :
					?><p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed or updated', 'doma' ); ?>:</p>
					...
					?><p class="lead success"><strong><?php esc_html_e( 'Great! All plugins has already been installed and up to date.', 'doma' ) ?></strong></p><?php
				endif;
				?><p><?php printf( esc_html__( 'Note that you can add and remove plugins from within %sWordPress admin dashboard%s whenever you want.', 'doma' ), '<a href="'.admin_url('plugins.php').'">', '</a>' ); ?></p>
				<p class="envato-setup-actions step">
					<?php
						if ($count) :
							?><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_plugins"><?php esc_html_e( 'Install plugins', 'doma' ); ?></a><?php
						else :
							?><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large"><?php esc_html_e( 'Continue', 'doma' ); ?></a><?php
						endif;
					?>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large"><?php esc_html_e( 'Skip this step', 'doma' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
		...
		protected function default_content()
		{
			...
			?><h1><?php esc_html_e( 'Import Sample Content', 'doma' ); ?></h1>
			<form action="" method="post"><?php
				if ( $this->is_possible_upgrade() ) :
					?><p><?php esc_html_e( 'It looks like you already have content imported. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'doma' ); ?></p><?php
				else :
					?><p><?php esc_html_e( 'It&#8217;s time to import some sample content for your new website. Tick the checkboxes of the contents you would like to import. Once imported, the content can be managed within the WordPress admin dashboard.', 'doma' ); ?></p><?php
				endif;
				...
				<br>
				<p><strong><em><?php esc_html_e('If you don&#8217;t know what content you should tick, it&#8217;s good idea to leave everything as default.', 'doma') ?></em></strong></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_content"><?php esc_html_e( 'Continue', 'doma' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large"><?php esc_html_e( 'Skip this step', 'doma' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form><?php
		...
		protected function demo()
		{
	        ...
	        ?><form action="" method="post">
				<h2><?php esc_html_e('Select site demo', 'doma') ?></h2>
				...
				<p><em><?php esc_html_e('If you are unsure to choose which style, just leave to it default. This works best on fresh new sites.', 'doma') ?></em></p>
	            <p class="envato-setup-actions step">
	                <input type="submit" class="button-primary button button-large" value="<?php esc_attr_e( 'Continue', 'doma' ); ?>" name="save_step" />
	                <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large"><?php esc_html_e( 'Skip this step', 'doma' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
	            </p>
	        </form><?php
		...
		protected function ready()
		{
			...
			?><h1 style="color:#7eb62e;"><?php esc_html_e( 'Congratulations! Your website is ready!', 'doma' ); ?></h1>
			<p><?php esc_html_e('Theme has been installed successfully. Go back to your WordPress dashboard to customize the default content and appearance to suit your needs.', 'doma') ?></p>
			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2 style="padding-bottom:10px"><?php esc_html_e( 'Next Steps', 'doma' ); ?></h2>
					<ul>
						<li class="setup-product">
							<a class="button button-primary button-large" href="https://themeforest.net/downloads" target="_blank"><?php esc_html_e( 'Rate this theme', 'doma' ); ?></a>
						</li>
						<li class="setup-product"><a class="button button-large" href="<?php echo esc_url( home_url('/') ); ?>"><?php esc_html_e( 'View your website', 'doma' ); ?></a></li>
						<li class="setup-product"><a class="button button-large" href="<?php echo esc_url(admin_url($customize_uri)); ?>"><?php esc_html_e( 'Customize your website', 'doma' ); ?></a></li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2 style="padding-bottom:10px"><?php esc_html_e( 'More Steps', 'doma' ); ?></h2>
					<ul>
						<li class="howto" style="font-style:normal">
							<a href="https://codex.wordpress.org/New_To_WordPress_-_Where_to_Start" target="_blank"><?php esc_html_e( 'Learn WordPress', 'doma' ); ?></a>
						</li>
						<li class="documentation">
							<a href="http://doc.zootemplate.com/<?php echo get_option('template') ?>" target="_blank"><?php esc_html_e( 'Read Documentation', 'doma' ); ?></a>
						</li>
						<li class="documentation">
							<a href="<?php echo esc_url('https://themeforest.net/page/item_support_policy') ?>" target="_blank"><?php esc_html_e( 'Item Support Policy', 'doma' ); ?></a>
						</li>
						<li class="support">
							<a href="<?php echo esc_url('http://member.zootemplate.com/helpdesk') ?>" target="_blank"><?php esc_html_e( 'Get Help and Support', 'doma' ); ?></a>
						</li>
					</ul>
				</div>
			</div><?php
		...
		protected function get_base_content()
		{
			...
			if ( !empty($content) && is_array($content) ) {
				foreach ($content as $slug => $data) {
					...
					$content[$slug]['description'] = esc_html__('Sample', 'doma') . ' ' . $slug . ' ' . esc_html__('data.', 'doma');
					$content[$slug]['pending'] = esc_html__('Pending', 'doma');
					$content[$slug]['installing'] = esc_html__('Installing', 'doma');
					$content[$slug]['success'] = esc_html__('Success', 'doma');
			...
			if ( file_exists($base_dir . 'widgets.wie') ) {
				$content['widgets'] = array(
					'title'            => esc_html__('Widgets', 'doma'),
					'description'      => esc_html__('Sample widgets data.', 'doma'),
					'pending'          => esc_html__('Pending', 'doma'),
					'installing'       => esc_html__('Installing', 'doma'),
					'success'          => esc_html__('Success', 'doma'),
			...
			if ( file_exists($base_dir . 'slider.zip') ) {
				$content['sliders'] = array(
					'title'            => esc_html__('Sliders', 'doma'),
					'description'      => esc_html__('Sample sliders data.', 'doma'),
					'pending'          => esc_html__('Pending', 'doma'),
					'installing'       => esc_html__('Installing', 'doma'),
					'success'          => esc_html__('Success', 'doma'),
			...
			if ( file_exists($base_dir . 'customizer.dat') ) {
				$content['options'] = array(
					'title'            => esc_html__('Options', 'doma'),
					'description'      => esc_html__('Sample theme options data.', 'doma'),
					'pending'          => esc_html__('Pending', 'doma'),
					'installing'       => esc_html__('Installing', 'doma'),
					'success'          => esc_html__('Success', 'doma'),
			...
			if ( file_exists($base_dir . 'settings') ) {
				$content['settings'] = array(
					'title'            => esc_html__('Settings', 'doma'),
					'description'      => esc_html__('Sample theme and plugins&#8217; settings data.', 'doma'),
					'pending'          => esc_html__('Pending', 'doma'),
					'installing'       => esc_html__('Installing', 'doma'),
					'success'          => esc_html__('Success', 'doma'),
