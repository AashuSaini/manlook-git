...
<div class="wrap-price">
	<p class="price">
		...
		echo ent2ncr($product->get_price_html()); ?></p>
</div>
    <div class="wrap-meta-content">
        ...
        if (!$product->is_in_stock()) {
            ?>
            <span class="out-stock-label stock"><?php esc_html_e('Out of stock', 'woocommerce'); ?></span>
            <?php
        } else {
            if (get_option('woocommerce_notify_low_stock_amount') > $product->get_stock_quantity() && $product->get_stock_quantity()) {
                ?>
                <span class="low-stock-label stock"><?php esc_html_e('Low in stock', 'woocommerce'); ?></span>
                <?php
            }else{
                ?>
