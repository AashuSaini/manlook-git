...
final class Zoo_Breadcrumbs
{
    ...
    private function theCatCrumb(\WP_Query $query, $crumb = '')
    {
        ...
        echo ent2ncr($this->getTypeCrumb($query) . $crumb . $label = esc_html__('Categorized as', 'doma') . ': ' . htmlspecialchars($query->queried_object->name, ENT_QUOTES));
    }
    /**
     * Tag archive crumb
     */
    ...
    {
        echo ent2ncr($this->getTypeCrumb($query) . esc_html__('Tagged with', 'doma') . ': ' . htmlspecialchars($query->queried_object->name, ENT_QUOTES));
    }
    /**
    ...
    {
        echo ent2ncr($this->getTypeCrumb($query) . esc_html__('Classified as', 'doma') . ': ' . htmlspecialchars($query->queried_object->name, ENT_QUOTES));
    }
    /**
    ...
    {
        echo ent2ncr($this->getTypeCrumb($query) . esc_html__('Posted in', 'doma') . ': ' . $query->query_vars['year']);
    }
    /**
    ...
    private function theMonthCrumb(\WP_Query $query)
    {
        ...
        echo ent2ncr($this->getCrumbLink( get_year_link($year), $year ) . $this->getSep() . esc_html__('Posted on', 'doma') . ': ' . $month);
    }
    /**
    ...
    {
        echo esc_html__('Posted by', 'doma') . ': ' . $query->queried_object->display_name;
    }
    /**
     * Single page crumb
     */
    ...
    {
        echo esc_html__('Search results for', 'doma') . ': ' . htmlspecialchars($query->query_vars['s'], ENT_QUOTES);
    }
    /**
     * 404 crumb
     */
    ...
    {
        echo esc_html__('404 Not Found', 'doma');
    }
    /**
     * Single post crumb
     */
