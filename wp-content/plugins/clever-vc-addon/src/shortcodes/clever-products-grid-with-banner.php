<?php
/**
 * Products Grid with Category Tabs Shortcode
 */
if (!function_exists('cvca_shortcode_products_grid_with_banner')) {
    function cvca_shortcode_products_grid_with_banner($atts, $content)
    {

        if (class_exists('WooCommerce')) {
            $atts = shortcode_atts(apply_filters('CleverProductsGridwithBanner_shortcode_atts',array(
                'title' => '',
                'img_banner' => '',
                'link' => '',
                'product_ids' => '',
                ...
                'css' => '',
            )), $atts, 'CleverProductsGridwithBanner');
            $html = cvca_get_shortcode_view( 'products-grid-with-banner', $atts, $content );
...
add_shortcode('CleverProductsGridwithBanner', 'cvca_shortcode_products_grid_with_banner');

add_action('vc_before_init', 'cvca_products_grid_with_banner_integrate_vc');

if (!function_exists('cvca_products_grid_with_banner_integrate_vc')) {
    function cvca_products_grid_with_banner_integrate_vc()
    {
        if (class_exists('WooCommerce')) {
            $asset_type =  array(
            ...
            vc_map(
                array(
                    'name' => esc_html__('Products Grid with Banner', 'cvca'),
                    'base' => 'CleverProductsGridwithBanner',
                    'icon' => 'icon-wpb-woocommerce',
                    'category' => esc_html__('CleverSoft Elements', 'cvca'),
                    'description' => esc_html__('Show multiple products grid tabs.', 'cvca'),
                    'params' => array(
                        array(
                            'type' => 'textfield',
                        ...
                        array(
                            'type' => 'attach_image',
                            'heading' => esc_html__('Banner', 'cvca'),
                            "admin_label" => true,
                        ...
                        array(
                            'type' => 'vc_link',
                            'heading' => esc_html__('Banner Link', 'cvca'),
                            "admin_label" => true,
                            'param_name' => 'link',
                            'description' => esc_html__('', 'cvca'),
                        ),
                        ...
                        array(
                            "type" => "dropdown",
                            "heading" => esc_html__("Choose default show category", 'cvca'),
...
add_filter( 'vc_autocomplete_CleverProductsGridwithBanner_product_ids_callback', array(
    $wc_class,
    'productIdAutocompleteSuggester',
), 10, 1 ); // Get suggestion(find). Must return an array
add_filter( 'vc_autocomplete_CleverProductsGridwithBanner_product_ids_render', array(
    $wc_class,
    'productIdAutocompleteRender',
), 10, 1 ); // Render exact product. Must return an array (label,value)
//For param: ID default value filter
add_filter( 'vc_form_fields_render_field_CleverProductsGridwithBanner_product_ids_param_value', array(
    $wc_class,
