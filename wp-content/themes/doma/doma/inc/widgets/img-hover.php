...
class zoo_imghover_widget extends WP_Widget
{
    ...
    {
        /* Widget settings. */
        $widget_ops = array('classname' => 'zoo_imghover_widget', 'description' => esc_html__('Easy add image for sidebar.', 'doma'));

        /* Widget control settings. */
        $control_ops = array('width' => 250, 'height' => 350, 'id_base' => 'zoo_imghover_widget');

        /* Create the widget. */
        parent::__construct('zoo_imghover_widget', esc_html__('Zoo: Image Sidebar', 'doma'), $widget_ops, $control_ops);
    }
    ...
    function widget($args, $instance)
    {
        ...
        $link = $instance['link'];
        $text_link = $instance['text_link'] != '' ?  esc_html($instance['text_link']) : '';
    ...
    function form($instance)
    {
        ...
        $defaults = array('title' => esc_html__('', 'doma'),
            'image' => '',
            'caption' => '',
            'link' => '#',
            'text_link' => '',
        ...
        <p id="<?php echo esc_attr($this->get_field_id('image').'-wrapp'); ?>">
            <label for="<?php echo esc_attr($this->get_field_id('image')); ?>"><?php esc_html_e('Image:', 'doma'); ?></label>
            ...
            <br>
            <input type="button" class="button button-primary custom_media_button" id="<?php echo esc_attr($this->get_field_id('image').'-button'); ?>" value="<?php esc_attr_e('Select Image','doma')?>" />
        </p>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('caption')); ?>"><?php esc_html_e('Caption:', 'doma'); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('link')); ?>"><?php esc_html_e('Link:', 'doma'); ?></label>
        ...
        <p>
            <label
                for="<?php echo esc_attr($this->get_field_id('text_link')); ?>"><?php esc_html_e('Text Link:', 'doma'); ?></label>
        ...
        <p>
            <label
                for="<?php echo esc_attr($this->get_field_id('target')); ?>"><?php esc_html_e('Open in new tabs:', 'doma'); ?></label>
            <select id="<?php echo esc_attr($this->get_field_id('target')); ?>"
                    name="<?php echo esc_attr($this->get_field_name('target')); ?>" class="widefat"
                    style="width:100%;">
                <option
                    value='yes' <?php if ('yes' == $instance['target']) echo 'selected="selected"'; ?>><?php esc_html_e('Yes', 'doma'); ?>
                </option>
                <option
                    value='no' <?php if ('no' == $instance['target']) echo 'selected="selected"'; ?>><?php esc_html_e('No', 'doma'); ?>
            </select>
        </p>
        <p>
            <label
                for="<?php echo esc_attr($this->get_field_id('img_align')); ?>"><?php esc_html_e('Align:', 'doma'); ?></label>
            <select id="<?php echo esc_attr($this->get_field_id('img_align')); ?>"
                    ...
                    style="width:100%;">
                <option
                    value='center' <?php if ('center' == $instance['img_align']) echo 'selected="selected"'; ?>><?php esc_html_e('Center', 'doma'); ?>
                </option>
                <option
                    value='left' <?php if ('left' == $instance['img_align']) echo 'selected="selected"'; ?>><?php esc_html_e('Left', 'doma'); ?>
                <option
                    value='right' <?php if ('right' == $instance['img_align']) echo 'selected="selected"'; ?>><?php esc_html_e('Right', 'doma'); ?>
            </select>
        </p>
        <p>
            <label
                for="<?php echo esc_attr($this->get_field_id('class')); ?>"><?php esc_html_e('Class:', 'doma'); ?></label>
