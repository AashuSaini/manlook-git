    ...
    class ZooCustomizeManager
    {
        ...
        function _ajaxReset()
        {
            if (!$this->wp_customize->is_preview()) {
                wp_send_json(array(
                    'success' => false,
                    'message' => esc_html__('Not previewing', 'doma')
                ));
            }
            if ( !check_ajax_referer('cust0miz3r-n0nc3', 'nonce', false)) {
                wp_send_json(array('success' => false, 'message' => esc_html__('Cheating?', 'doma')));
            }
            ...
            wp_send_json(array('success' => true, 'message' => esc_html__('Customize data reset successfully!', 'doma')));
        }
        /**
         * Import attachment
         */
        ...
        protected function sideLoadCustomizeImage($file)
        {
            if (!preg_match('/[^\?]+\.(jpe?g|jpe|gif|png)\b/i', $file, $matches)) {
                return new WP_Error('_invalid_attachment', esc_html__('Invalid attachment!', 'doma'));
            }
            $data = new \stdClass();
