<?php
/**
 * Products List with Category Tabs Shortcode
 */
if (!function_exists('cvca_shortcode_products_list_category_tabs')) {
    function cvca_shortcode_products_list_category_tabs($atts, $content)
    {

        if (class_exists('WooCommerce')) {
            $atts = shortcode_atts(apply_filters('CleverProductsListCategoryTabs_shortcode_atts',array(
                'title' => '',
                'accent_color' => '',
                ...
                'css' => '',
            )), $atts, 'CleverProductsListCategoryTabs');
            $html = cvca_get_shortcode_view( 'products-list-with-category-tabs', $atts, $content );
...
add_shortcode('CleverProductsListCategoryTabs', 'cvca_shortcode_products_list_category_tabs');

add_action('vc_before_init', 'cvca_products_list_category_tabs_integrate_vc');

if (!function_exists('cvca_products_list_category_tabs_integrate_vc')) {
    function cvca_products_list_category_tabs_integrate_vc()
    {
        if (class_exists('WooCommerce')) {
            $asset_type =  array(
            ...
            if (count($product_categories) > 0) {
                ...
                vc_map(
                    array(
                        'name' => esc_html__('Products Grid', 'cvca'),
                        'base' => 'CleverProductsListCategoryTabs',
...
add_filter( 'vc_autocomplete_CleverProductsListCategoryTabs_product_ids_callback', array(
    $wc_class,
    'productIdAutocompleteSuggester',
), 10, 1 ); // Get suggestion(find). Must return an array
add_filter( 'vc_autocomplete_CleverProductsListCategoryTabs_product_ids_render', array(
    $wc_class,
    'productIdAutocompleteRender',
), 10, 1 ); // Render exact product. Must return an array (label,value)
//For param: ID default value filter
add_filter( 'vc_form_fields_render_field_CleverProductsListCategoryTabs_product_ids_param_value', array(
    $wc_class,
