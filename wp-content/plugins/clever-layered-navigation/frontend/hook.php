...
add_filter('woocommerce_product_query', 'Zoo\Frontend\Hook\process_filter', 10, 1);
...
{
    $arg = $wc_query->query;
    if (isset($arg['paged']) && $arg['paged'] != '1') {
        $page_index = intval($arg['paged']);
...
function process_filter($wp_query)
{
    ...
    if (isset($_POST['zoo_ln_form_data']) && defined('DOING_AJAX') && DOING_AJAX) {
        ...
        $post_option = $_GET;
        if(isset($wp_query->query['product_cat']) && !isset($post_option['categories'])){
